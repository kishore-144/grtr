...
const nickPath = path.join(process.env.HOME || process.env.USERPROFILE, ".grtr-nicknames.json");

export function registerNickname(nickname) {
  const network = Object.values(os.networkInterfaces()).flat().find((iface) => iface.family === "IPv4" && !iface.internal);
