...
export async function sendFile(nickname, targetPath, medium) {
    ...
    if (!ip) {
        console.log(
            chalk.cyan(`🔍 Searching for '${nickname}' on local network...`)
        );
        ip = await discoverDevice(nickname, medium);
        if (!ip) {
            console.log(
                chalk.redBright(
                    `✖ Could not locate '${nickname}' on your network.`
                )
            );
            return;
        }
    ...
    if (stat.isDirectory()) {
        ...
        try {
            ...
            console.log(
                chalk.redBright("✖ Compression failed: ") +
    ...
    console.log(
        chalk.yellow(
            `📤 Transferring '${path.basename(fileToSend)}' to ${nickname} (${ip})...`
        )
    );
    ...
    try {
        ...
        console.log(
            chalk.redBright("✖ Transfer failed: ") + chalk.gray(err.message)
        );
    } finally {
...
export async function discoverDevice(nickname, medium) {
    ...
    const virtualPattern =
        /(vEthernet|virtual|hyper-v|wsl|docker|vmware|loopback|tunnel|isatap|pontoon)/i;
    ...
    if (candidates.length === 0) {
        candidates = Object.entries(interfaces)
            ...
            .filter(
                ({ addr }) => addr && addr.family === "IPv4" && !addr.internal
            );
    }
    ...
    const subnets = [
        ...new Set(
            candidates
                .map((c) =>
                    c.addr.address
                        ? c.addr.address.split(".").slice(0, 3).join(".")
                        : null
                )
                .filter(Boolean)
        ),
    ...
    console.log(
        chalk.gray(
            `🔎 Scanning ${queue.length} addresses with concurrency ${CONCURRENCY}...`
        )
    );
    ...
    const scanIp = async (ip) => {
        ...
        try {
            const resp = await axios.get(`http://${ip}:5050/whoami`, {
                timeout: TIMEOUT,
            });
            ...
            progressCounter++;
            if (progressCounter % PROGRESS_DOT_EVERY === 0)
    ...
    if (!foundIP) {
        console.log(
            "\n" +
                chalk.redBright(
                    `✖ No response from devices on scanned subnets.`
                )
        );
        return null;
    }
...
function zipFolder(src, dest) {
    return new Promise((resolve, reject) => {
        ...
        archive.on("warning", (err) => {
            if (err.code === "ENOENT")
                console.log(chalk.yellow("⚠ Archive warning:"), err.message);
            else reject(err);
        });
